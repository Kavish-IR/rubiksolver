...
def reshape_images(training_images, faces_per_image=6):
    ...
    all_squares = [square for img in faces for clmn in np.hsplit(
        np.flipud(img), 3) for square in np.vsplit(clmn, 3)]
    all_square_means_rgb = [np.mean(square, axis=(0, 1))
                            for square in all_squares]
    all_square_means_rgb = np.array(all_square_means_rgb).astype('uint8')
    all_square_means_yuv = cv.cvtColor(
        all_square_means_rgb.reshape(1, n_squares, 3), cv.COLOR_RGB2YUV)
...
def get_classifier():
    ...
    if not os.path.exists(pickled_model):
        ...
        clf_yuv = svm.SVC(gamma='scale', kernel='poly',
                          degree=5, probability=True)
