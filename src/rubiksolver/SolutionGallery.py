...
class SolutionGallery:
    def __init__(self, initial_state, solve_moves, i_start = 0):
        ...
        self.ax1 = self.c.cube_plot(ax=self.ax1, title_str='Initial State\n Use left/right arrow keys to navigate')

        # Set up connection to detect keys
        self.mpl_id  = self.fig.canvas.mpl_connect('key_press_event', self.handle_keypress)
    ...
    def handle_keypress(self, event):
        ...
        if event.key == 'right':
            if self.i < self.n_moves:
                ...
                self.c.cube_plot(ax=self.ax1, title_str='Final State\n(press q to quit)')
            plt.draw()
