...
class ManualSquareExtractor:
    ...
    def connect(self):
        # Add event listeners
        self.clicker = self.c.mpl_connect(
            "button_press_event", lambda e: self.button_press(e))
        self.presser = self.c.mpl_connect(
            "key_press_event", lambda e: self.key_press(e))
        self.follower = self.c.mpl_connect(
            "motion_notify_event", lambda e: self.followmouse(e))
    ...
    def remove_last_square(self):
        ...
        if len(self.square_list) > 0:
            self.update_rightmost_point(
                self.square_list[-1][2, :].reshape((1, 2)))
        else:
            self.update_rightmost_point(np.zeros((1, 2)))
    ...
    def add_square(self, event):
        if (event.xdata <= self.rightmost_pt[0, 0] or
            ...
            print(
                "Can't release here. Release down / right of : {0}".format(self.rightmost_pt))
        else:
    ...
    def grab_squares(self):
        ...
        d = 0.5 * (np.linalg.norm(mus[1, :] - mus[0, :]) +
                   np.linalg.norm(mus[2, :] - mus[1, :]))
        a = np.sqrt(1 / 2) * d
