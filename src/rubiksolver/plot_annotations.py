...
def basic_triangle(D):
    ...
    for i in range(nh+1):
        Y_tri[:, i]     = np.linspace(0, (h_tri / b_tri) * (b_tri + X_tri[0, i]), n) - h_tri / 3
        Y_tri[:, n-i-1] = Y_tri[:, i]
...
def basic_arrow():        
    ...
    Y_tri = np.zeros_like(X_tri)
    for i in range(nh+1):
        Y_tri[:, i]     = np.linspace(0, -(h_tri / w_tri) * (w_tri + X_tri[0, i]), n)
        Y_tri[:, n-i-1] = np.linspace(0, -(h_tri / w_tri) * (w_tri + X_tri[0, i]), n)
    X_tri = 4.5 + X_tri
...
def add_move_annotation(ax, move):
    ...
    if move_type in ['U', 'D', 'L', 'R', 'F', 'B']:        
        for i in range(3):
            X_tri, Y_tri, Z_tri = basic_triangle(D = np.array([2, -2 + 2*i, 4.5]))
        
            if orientation == 'CCW':        
                Y_tri = -Y_tri
                
            if move_type == 'R':
                ax.plot_surface(X_tri,  Y_tri,  Z_tri, color='k', edgecolor='none', shade=False)
                ax.plot_surface(X_tri, -Z_tri,  Y_tri, color='k', edgecolor='none', shade=False)
                ax.plot_surface(X_tri, -Y_tri, -Z_tri, color='k', edgecolor='none', shade=False)
                ax.plot_surface(X_tri,  Z_tri, -Y_tri, color='k', edgecolor='none', shade=False)
                ...
                ax.plot_surface( Y_tri, -X_tri,  Z_tri, color='k', edgecolor='none', shade=False)
                ax.plot_surface( Z_tri, -X_tri, -Y_tri, color='k', edgecolor='none', shade=False)    
                ax.plot_surface(-Y_tri, -X_tri, -Z_tri, color='k', edgecolor='none', shade=False)
                ax.plot_surface(-Z_tri, -X_tri,  Y_tri, color='k', edgecolor='none', shade=False)    
                ...
                ax.plot_surface(-Y_tri, -Z_tri, X_tri, color='k', edgecolor='none', shade=False)
                ax.plot_surface( Z_tri, -Y_tri, X_tri, color='k', edgecolor='none', shade=False)
                ax.plot_surface( Y_tri,  Z_tri, X_tri, color='k', edgecolor='none', shade=False)
                ax.plot_surface(-Z_tri,  Y_tri, X_tri, color='k', edgecolor='none', shade=False)
